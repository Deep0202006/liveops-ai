...
if (source("/api/(.*)") < 0 || source("/api/(.*)") > source("/command")) throw new Error("API rewrite must precede SPA rewrites");
for (const route of ["/command", "/asset/(.*)", "/maintenance", "/compare", "/model", "/lab", "/app", "/app/(.*)"]) if (rewrites[source(route)]?.destination !== "/index.html") throw new Error(`SPA deep-link missing: ${route}`);
